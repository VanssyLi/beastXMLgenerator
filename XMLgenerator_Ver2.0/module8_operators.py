...
def build_operator(beast, partition_file, fasta):
    ...
    subsModel_HKY(operators, partition_file)
    strict_clock(operators, partition_file)
    treeModel(operators)
    skygrid(operators)
    tip(operators, fasta)


def subsModel_HKY(operators, partition_file):
...
def strict_clock(operators, partition_file):
    for p, exclude, every3, susb in f.get_partition_info(partition_file):
        p = f.Partition(p, exclude, every3, susb)
